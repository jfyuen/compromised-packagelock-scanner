...
def find_packagelocks():
    if sys.platform != "darwin":
        return []

    cmd = ['mdfind', '-name', 'package-lock.json']
    result = subprocess.run(cmd, capture_output=True, text=True, check=True)

    return [path for path in result.stdout.strip().split('\n') if path]

def check_packages_in_lockfile(packages: list, file_path: str):
    logging.debug(f"checking packages in '{file_path}'")
    with open(file_path, 'r') as f:
        content = f.read() # just check if file is empty to prevent json errors
        if not content:
            return
        data = json.loads(content)

    all_packages = data.get("packages") # package-lock v2/v3
    if not all_packages:
        all_packages = data.get("dependencies") # package-lock 1
    if not all_packages:
        return
...
if __name__ == "__main__":
    ...
    for file_path in find_packagelocks():
        try:
            check_packages_in_lockfile(PACKAGES, file_path)
        except Exception as e:
            logging.exception(f"error when checking {file_path}")
            continue